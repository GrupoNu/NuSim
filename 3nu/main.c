...
U31=gsl_complex_add_real(POLAR(-c12*c23*s13,d_CP),s12*s23),  U32=gsl_complex_add_real(POLAR(-s12*c23*s13,d_CP),-c12*s23), U33=REAL(c23*c13);
    ...
    MAT(M, 0, 0, REAL(0.0));
                              MAT(M, 1, 1, REAL(DM2_21));
                                                           MAT(M, 2, 2, REAL(DM2_31));

    gsl_matrix_complex *h0 = gsl_matrix_complex_alloc(GERAC, GERAC);
    action(U, M, h0);   /* h0 = 1/2  U . M . U^dagger */
...
void action(const gsl_matrix_complex *A, const gsl_matrix_complex *B, gsl_matrix_complex *C) {
    ...
    gsl_blas_zgemm(CblasNoTrans, CblasConjTrans,    REAL(0.5)   , A_mult_B, A, GSL_COMPLEX_ZERO, C);        /* C = 1/2 A.B.A^dagger */
